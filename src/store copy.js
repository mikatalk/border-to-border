...
//   const keysA = Object.create(null)
//   const keysB = Object.create(null)
...
//     // count column A only
//     keysA[value[0]] = (keysA[value[0]] || 0) + 1
//     // count column B only
//     keysB[value[1]] = (keysB[value[1]] || 0) + 1
...
//   // generate normalized values for each column
//   for (let key in linksColumns) {
//     const column = linksColumns[key]
//     const min = Math.min(...column.map(value => parseFloat(value)))
//     const max = Math.max(...column.map(value => parseFloat(value)))
//     if (!isNaN(min) && !isNaN(max)) {
//       linksNormalizedColumns[key] = column.map(value => (parseFloat(value)-min) / (max-min))
//     }
  
//   }
//   commit('setLinksColumnsData', {linksColumns, linksNormalizedColumns})
...
export default new Vuex.Store({
  ...
  actions: {
    ...
    initialize ({state, commit, dispatch}) {
      ...
      setTimeout(() => {
        ...
        const nodes = Object.keys(mockData.countries).map((code, index) => {
          const country = mockData.countries[code];
          const node = Object.create(null)
          node.id = code
          node.name = country.name
          node.type = 1;
          node.linksCount = country.linksCount;
          node.radius = 0.02;
          return node;
        });
  ...
  // add links
  let links = mockData.borders.map(value => {
    if (value[0] != '' && value[1] != '') {
      const link = Object.create(null)
  ...
  // commit('setLinksColumnsData', {
  //   linksColumns: new Array(links.length).fill(1),
  //   linksNormalizedColumns: new Array(links.length).fill(1)
  // })
        commit('setNodesAndLinks', {nodes, links})
